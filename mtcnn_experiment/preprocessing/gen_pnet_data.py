...
def gen_pnet_data(data_dir, anno_file, prefix):
    #创建3个文件夹neg、pos、part， 12是随机截取区域的size的最小值
    ...
    for dir_path in [neg_save_dir, pos_save_dir, part_save_dir]:  # make dirs
...
#         os.mkdir(save_dir)
    ...
    with open(anno_file, 'r') as f: # 读取每个训练集样本的标注
    ...
    for annotation in annotations:
        ...
        boxes = np.array(bbox, dtype=np.int32).reshape(-1, 4)  # N*4 dim array，每个box由4个数值组成
        ...
        neg_num = 0
        while neg_num < 50: # 每张img随机裁剪50次，iou < 0.3 就作为负样本
            # 随机裁剪img中的一部分
            ...
            # 当iou的最大值小于0.3，就作为负样本
            if np.max(Iou) < 0.3:
                # Iou with all gts must below 0.3
                save_file = os.path.join(neg_save_dir, "%s.jpg" % n_idx)  # save neg image
                f2.write(save_file + ' 0\n')
                cropped_im = img[ny: ny + size, nx: nx + size, :]
                resized_im = cv2.resize(cropped_im, (12, 12), interpolation=cv2.INTER_LINEAR) # 保存的作用是什么？
                cv2.imwrite(save_file, resized_im)
                n_idx += 1 # 所有img的负样本 + 1
                neg_num += 1 # 一个img的负样本 + 1

        # 遍历标注 boxes，在标注boxes的基础加上offset生成负样本、正样本、部分样本
        for box in boxes:
            ...
            # ignore small faces
            # in case the ground truth boxes of small faces are not accurate， 排除异常数据
            ...
            # 每个box随机生成20个box，Iou>=0.65的作为positive examples，0.4<=Iou<0.65的作为part faces，其他忽略
            for i in range(20):
                ...
                if IoU(crop_box, box_) >= 0.65: # 保存正样本
                    save_file = os.path.join(pos_save_dir, "%s.jpg" % p_idx)
                    f1.write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' %
                             (offset_x1, offset_y1, offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im) # 保存的作用？
                    p_idx += 1
                elif IoU(crop_box, box_) >= 0.4: # 保存 part_example
...
def parse_args():
    ...
    parser.add_argument('--prefix_path', dest='prefix_path', help='annotation file image prefix root path',
                        default='../dataset/WiderFace/WIDER_train/images', type=str) # 根据自己电脑上的训练集路径设置
...
if __name__ == '__main__':
    args = parse_args()
    gen_pnet_data(args.traindata_store, args.annotation_file, args.prefix_path) # 训练样本文件保存路径、原始标注文件路径、训练集路径
