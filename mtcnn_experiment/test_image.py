...
if __name__ == '__main__':
    mtcnn_detector = MtcnnDetector(min_face_size=24, use_cuda=True)  # 加载模型参数，构造检测器
    logger.info("Init the MtcnnDetector.")
    project_root = pathlib.Path()
    inputPath = project_root / "data" / "test_images"
    outputPath = project_root / "data" / "test_images_result"
    ...
    with torch.no_grad():
        ...
        for num, input_img_filename in enumerate(inputPath.iterdir()):
            ...
            logger.info("Predict complete. Save image to {}.".format(str(savePath)))
